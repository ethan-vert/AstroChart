...
import { ElementWrapper, DocumentWrapper, type SVGElement } from './svg-factory'
...
class SVG {
  ...
  constructor(elementId: string, width: number, height: number, settings: Settings, doc?: DocumentWrapper) {
    ...
    this.document = doc || (globalThis as any).document
    
    let rootElement: ElementWrapper | null = null
    if (elementId !== '') {
      rootElement = this.document.getElementById(elementId)
      if (rootElement == null && this.document === (globalThis as any).document) {
  ...
  getSymbol(name: string, x: number, y: number): ElementWrapper {
    ...
    const symbol = this.settings.CUSTOM_SYMBOL_FN(name, x, y, this.context) as any
    if (symbol == null || symbol === undefined) return this._getSymbol(name, x, y)

    return symbol as ElementWrapper
  }
